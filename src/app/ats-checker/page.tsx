...
export default function ATSCheckerPage() {
  ...
  const analyzeResume = async () => {
    ...
    const uploadRes = await fetch("http://127.0.0.1:8000/ats/upload", {
      method: "POST",
      body: formData,
    });

    const uploadJson = await uploadRes.json();
    setResumeText(uploadJson.text);

    // 2) Score ATS
    const scoreRes = await fetch("http://127.0.0.1:8000/ats/score", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: uploadJson.text }),
    });

    const scoreJson = await scoreRes.json();
    setScore(scoreJson.score);
    setIssues(scoreJson.issues);
    setDetectedRole(scoreJson.role_token || scoreJson.detected_role || "unknown");
  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white p-10">
      ...
      <p className="text-zinc-400 mb-8">
        Upload your resume to get a detailed ATS score and improvement suggestions.
      </p>

      {/* Upload Box */}
      <div className="bg-zinc-800 p-6 rounded-xl border border-zinc-700 max-w-xl">
        <label className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-zinc-600 rounded-xl cursor-pointer hover:border-indigo-500 transition">
        ...
        <button
          ...
          className="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 px-4 py-2 rounded-lg font-semibold"
        >
      ...
      {score !== null && (
        <div className="mt-10">
          <h2 className="text-3xl font-bold mb-4">Your ATS Score</h2>
          {detectedRole && (<p className="text-zinc-300">Detected Role: <strong>{detectedRole}</strong></p>)}
          {/* Score Circle */}
          <div className="flex items-center gap-6">
            <div
              className="w-40 h-40 rounded-full flex items-center justify-center text-4xl font-bold border-8"
              style={{
                borderColor:
            ...
            <div>
              ...
              {/* Missing Sections */}
              {issues.missing_sections.length > 0 ? (
                <p className="text-red-400">
                  <AlertCircle className="inline w-5 h-5 mr-2" />
                  Missing Sections: {issues.missing_sections.join(", ")}
                </p>
              ) : (
              ...
              Grammar Issues: {issues?.grammar_issues?.length ?? 0}
              </p>

              {/* Keyword Count */}
              <p className="text-zinc-300">
                Keywords Found: {issues.keyword_count}
              </p>

              {/* Bullet & Word Count */}
              <p className="text-zinc-300">
                Bullets: {issues.bullet_count} • Words: {issues.word_count}
              </p>
            </div>
          </div>
        </div>
